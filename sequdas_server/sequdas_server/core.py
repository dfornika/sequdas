...
import sequdas_server.sample_sheet
...
def usage():
    usage = """
    sequdas_server.py -i <input_directory>
...
def copy_reporter(out_dir, run_folder_name):
    config = sequdas_config()
    ssh_host_report = config['reporter']['reporter_ssh_host']
    QC_img_dir= config['reporter']['qc_dir']
    ...
    for sample in sample_list:
        ...
        try:
           ...
           if(keep_kraken is False):
               p3_6 = subprocess.Popen(['rm', '-rf', kraken_result_file])
               a=p3_5.wait()
        except:
