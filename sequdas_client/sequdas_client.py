...
def main():
    ...
    sequdas_client.check.check_run_folders(run_dir_lists)
    machine = config['basic']['sequencer']
    ssh_private_key = config['local']['ssh_primate_key']
    logfile_dir = config['basic']['logfile_dir']    
    logfile_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), config['basic']['logfile_dir'])
    logfile_dir = check_path_with_slash(logfile_dir)
    logfile = logfile_dir + machine + "_sequdas_log.txt"
    send_email_switch = str2bool(config['basic']['send_email'])
    # Remote setting
    data_server = config['server']['server_ssh_host']
    data_repository = config['server']['server_data_dir']
    ...
    check_server_folder(data_server, data_dir)
    # Email setting
    gmail_user = config['email_account']['gmail_user']
    gmail_pass = config['email_account']['gmail_pass']
    admin_emails = config['basic']['admin_email']
                ...
                if status_result == "Status_OK":
                    ...
                    for step in range(1, 6):
                        if step == 5 and uploader == 0:
                            ...
                            try:
                                shell = spur.SshShell(hostname="sabin.bcgsc.ca", username="miseq", private_key_file=ssh_private_key)
